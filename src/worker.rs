                                ...
                                AlcoholStatus::Uninitialized => {
                                    ...
                                    if let Ok(json) = serde_json::to_string(&response) {
                                        ...
                                        tokio::spawn(async move {
                                            let _ = state.send_message(SocketType::Client, Message::text(json_client)).await;
                                        });
                                        tokio::spawn(async move {
                                            let _ = state.send_message(SocketType::Pump, Message::text(json_pump)).await;
                                        });
                                    }
                                }
                                AlcoholStatus::Calibrated => {
                                    ...
                                    match alcohol.tick() {
                                        ...
                                        Option::Some(amount)=> {
                                            ...
                                            tokio::spawn(async move {
                                                if let Ok(json) = serde_json::to_string(&response) {
                                                    let _ = state.send_message(SocketType::Pump, Message::text(json)).await;
                                                }
                                            });
                                        },
                                    }
